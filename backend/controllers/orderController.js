...
export const placeOrder = async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      line_items,
      success_url: `${process.env.FRONTEND_URL || "http://localhost:5173"}/verify?success=true&orderId=${order._id}`,
      cancel_url: `${process.env.FRONTEND_URL || "http://localhost:5173"}/verify?success=false&orderId=${order._id}`,
    });
...
export const verifyPayment = async (req, res) => {
  try {
    ...
    if (success === "true" || success === true) {
      ...
      await order.save();
      return res.json({ success: true, data: order, message: "Payment verified" });
    } else {
      return res.json({ success: false, data: order, message: "Payment not completed" });
    }
  } catch (err) {
